...
df_stat= df.groupby(['Status'])[num].mean().reset_index().round(2)  # get the statistics for each status to display on dashboard
...
if status=='Developed':
    ...
    with col1:
        st.markdown(f'''
            <div class="metric">
                <h3>Average Life Expectancy</h3>
                <p>{df_developing['Life expectancy'][1]}</p>
            </div>
        ''', unsafe_allow_html=True)
        st.write(" ")
        st.markdown(f'''
            <div class="metric">
                <h3>Average Adult Mortality</h3>
                <p>{df_developing['Adult Mortality'][1]}</p>
            </div>
        ''', unsafe_allow_html=True)
        st.write(" ")
    ...
    with col2:
        st.markdown(f'''
            <div class="metric">
                <h3>Average Population</h3>
                <p>{df_developing['Population'][1]}</p>
            </div>
        ''', unsafe_allow_html=True)
